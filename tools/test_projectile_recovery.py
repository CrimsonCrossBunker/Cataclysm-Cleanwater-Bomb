#!/usr/bin/env python3
"""Exercise production recovery code; ballistic tests are in Catch2."""
import os
...
class ProjectileRecoveryTest(unittest.TestCase):
    def test_recovered_round_keeps_metadata(self):
        source = Path(os.environ.get('CCB_RANGED_SOURCE',
                      ROOT / 'src/ranged.cpp')).read_text()
        block = braced_block(source, source.index(
            'if( recover && !fx.count( ammo_effect_IGNITE )'))
...
enum { ammo_effect_IGNITE, ammo_effect_EXPLOSIVE,
       ammo_effect_ACT_ON_RANGED_HIT };
struct item {
...
int main() {
    ...
    assert(proj.drop.label=="marked" && proj.drop.damage==1000);
    assert(proj.drop.is_favorite);
