...
try:
    ...
    from .generate_platform_coverage import build_coverage, serialize_coverage
except ImportError:
    from generate_platform_contract import build_contract, parse_luals_declarations  # type: ignore
    from generate_platform_coverage import build_coverage, serialize_coverage  # type: ignore
...
class PlatformCoverageGeneratorTest(unittest.TestCase):
    def test_synchronization_coverage_matches_declared_native_root(self) -> None:
        ...
        self.assertIn("platform_luals_native_registration", coverage["coverage_kind"])
    ...
    def test_unmatched_native_root_is_reported(self) -> None:
        ...
        inventory = {"export_roots": [{"id": "other", "lua_name": "OtherRoot"}]}
        coverage = build_coverage(contract, inventory)
        self.assertFalse(coverage["platform_sync"]["synchronized"])
        self.assertEqual(coverage["platform_sync"]["unmatched_native_export_roots"], ["OtherRoot"])
