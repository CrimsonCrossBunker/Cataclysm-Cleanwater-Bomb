...
| sensitive()     |  ✅  |   ✅   | u, n  | Return or set the character's current sensitivity to external stimuli. Values below 100 dull and values above sharpen reactions such as pain gained, focus recovery and wetness morale. The value drifts towards its equilibrium over time.<br/><br/>Example:<br/>`{ "math": [ "u_sensitive() -= 10" ] }` Lowers the alpha talker's sensitivity by 10<br/>|
| sensitive_mod()     |  ✅  |   ✅   | u, n  | Return or set the equilibrium that current sensitivity drifts towards. Clamped to 0..500.<br/><br/>Example:<br/>`{ "math": [ "u_sensitive_mod() += 5" ] }` Raises the alpha talker's sensitivity equilibrium by 5<br/>|
