...
| #4 | 缺失 L3 数据 | creature(CreatureView) / light scalar / memorized tile / per-tile version（注：field/item/vpart 已在 #281/#285/#286 完成） |
...
| 2 ViewSnapshot | 🟡 部分完成。**已交付**: ① 静态层(terrain/furniture/trap/part_con/graffiti)从 cache 读(#272);② `view_snapshot` 类型带区域+观察者参数(#272);③ 瞬态视觉层 2D ✅ 完成;④ field/item/vpart 每帧捕获(#281/#285/#286);⑤ L3/L1-L2 拆分(#294,gap #2/#3 RESOLVED)。**未交付**: creature(活体对象,故意跳过);light scalar;memorized tile;per-tile version;序列化。 |
...
- 剩余未交付：creature(CreatureView)故意跳过、light scalar、memorized tile、per-tile version、序列化（gap #4 子集 + #5/#6/#7）
- SDL 线程亲和 + 输入命令回传 + UI 模态循环去阻塞是异步的硬前置
- **ViewSnapshot 入参已基本完整**（terrain/furniture/trap/graffiti/field/item/vpart 均已捕获），RenderBackend 接口可在现有基础上设计
